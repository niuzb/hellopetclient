...
std::map<int,NPCData *>* BoneNPCManager::LoadNPCConfigInfoFromLua(int levelId)
{
		...
		iter != pNpcBasicInfo->end();iter++)
	{
		...
		const char * type_flag_icon = LuaTinkerManager::Get()->getLuaConfig<const char *>("npc", "npcs", typeId, "TypeFlagIcon");
...
NPCData * BoneNPCManager::GetOneNpcBasicData(unsigned int npc_id,bool bFromLua /* = false */)
{
	...
	else
	{
		...
		const char * type_flag_icon = LuaTinkerManager::Get()->getLuaConfig<const char *>("npc", "npcs", npc_id, "TypeFlagIcon");
