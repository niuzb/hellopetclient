...
public:
	...
	//-----------------------------------------------------------------------
	void		addOtherPlayer(unsigned int uid, const char* userName, unsigned int type, cocos2d::CCPoint pt, unsigned int orient, unsigned int battleSide);
	void		removeOtherPlayer(unsigned int uid);
	void		updateOtherPlayer(unsigned int uid, cocos2d::CCPoint pt, unsigned int animID, bool bFlip);
	void		DoOtherPlayerSkill(unsigned int uid, unsigned int skillID, const CCPoint& rolePos, const CCPoint& roleDir, const CCPoint& aimDir, const CCPoint& monsterPos, bool bAnimMonster);
	void		DoElfSkill(unsigned int uid, unsigned int skillID, const CCPoint& rolePos, const CCPoint& roleDir, const CCPoint& aimDir, const CCPoint& monsterPos, bool bAnimMonster);
...
public:
	struct WAKL_INFO
	{
		struct WAKL_PACK
		{
			WAKL_PACK():bUsed(false),uid(0), animID(-1), bFlip(false){}
			unsigned int uid;
			cocos2d::CCPoint pt;
			unsigned int animID;
			bool bFlip;
	...
	std::map<unsigned int , WAKL_INFO> m_walkInfo;

	//---------------------------------------------------------------------------------
//				other players record
	//---------------------------------------------------------------------------------	
	std::map<uint32_t, OthersPlayerInfo>	m_othersPlayerInfo;
...
private:	
	...
	//---------------------------------------------------------------------------------	
	int								m_pathOffset;
	std::vector<MAP_POINT_INFO>		m_TransPointInfoList;
	std::vector<MAP_NPC_INFO>		m_npcInfoList;
