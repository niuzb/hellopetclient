...
SpriteTransPoint* LevelObjectCreator::sCreateTransPoint(const MAP_POINT_INFO& info)
{
	...
	SpriteTransPoint* transPoint = SpriteTransPoint::node(name, info.point, info.ID);
...
SpriteSeer* LevelObjectCreator::sCreateSeer(unsigned int id,const char* name, int type, bool isPlayerSet)
{
	SpriteSeer* seer = HeroRoleManager::Get()->createRole(id, name, type, isPlayerSet);	
