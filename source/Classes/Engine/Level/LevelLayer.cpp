...
bool LevelLayer::initLevel(unsigned int levelId, sLevelData* data,bool bStoryMode /* = false */)
{
	...
	touchEffect = ParticleManager::Get()->createEffectSprite(204,"");
...
void LevelLayer::destroyLayer()
{
	...
	NPCManager::sharedManager()->removeNpcsFromMap(m_id);
...
//------------------------------------------------------------------------
void LevelLayer::addOtherPlayer(unsigned int uid, const char* userName,unsigned int petType, cocos2d::CCPoint pt, unsigned int orient, unsigned int battleSide)
{
...
void LevelLayer::removeOtherPlayer(unsigned int uid)
{
	m_levelBuilder->removeOtherPlayer(uid);
}

SpriteSeer* LevelLayer::getPlayer(unsigned int uid)
{
...
void LevelLayer::updateOtherPlayer(unsigned int uid, cocos2d::CCPoint pt, unsigned int animID, bool bFlip)
{
	if(UserData::Get()->getUserId() == uid)
	{
		return;
	}
	std::map<uint32_t, OthersPlayerInfo>::iterator itor = m_othersPlayerInfo.find(uid);
	...
	{
		CCLog("can't find the player id %d", uid);
	}
}


void LevelLayer::DoOtherPlayerSkill( unsigned int uid, unsigned int skillID , const CCPoint& rolePos,  const CCPoint& roleDir, const CCPoint& aimDir, const CCPoint& monsterPos, bool bAnimMonster)
{
	if(UserData::Get()->getUserId() == uid)
	{
		return;
...
void LevelLayer::DoElfSkill( unsigned int uid, unsigned int skillID , const CCPoint& rolePos,  const CCPoint& roleDir, const CCPoint& aimDir, const CCPoint& monsterPos, bool bAnimMonster)
{
	if(UserData::Get()->getUserId() == uid)
	{
		return;
	...
	if (itor != m_othersPlayerInfo.end())
	{
		...
		if (playerInfo.seer)
		{
			...
			{
				if(iter->second->GetAttackedHero() == playerInfo.seer)
				{
					pSpriteElf = iter->second;
					break;
				}
			}
