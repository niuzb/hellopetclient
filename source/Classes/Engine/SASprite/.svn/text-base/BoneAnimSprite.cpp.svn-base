...
void BoneAnimSprite::SetAnim(int animId, int nloop, bool isLoop)
{
	...
	if (role_iter != armatureNameAndActorMap->end())
	{
		...
		CCConnectionData::sharedConnectionData()->getColliderData(m_curRoleId,animId,moveAreaData);
		CCConnectionData::sharedConnectionData()->getViewRectData(m_curRoleId,armature_name,viewAreaData);
		SetAnimFlipX(false);	
