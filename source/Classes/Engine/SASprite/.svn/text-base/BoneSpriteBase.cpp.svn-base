							  ...
							  BoneSpriteBase *listerner /* = 0 */)
{
	...
	if (m_animBone)
	{
		...
		if (0 != curRoleActor)
		{
			curRoleActor->setParentRole(0);
		}

		SetColliderBoxShowOrNot(false);
...
void BoneSpriteBase::SetColliderBoxShowOrNot(bool bShow)
{
	m_bShowColliderBox = bShow;
}
...
void BoneSpriteBase::drawEditorRect()
{
	if (m_bShowColliderBox)
	{
		...
		rect = getRelativeParentColliderRect();
		Painter::drawRect(rect,255, 0, 255, 255);
		rect = getRelativeParentViewRect();
...
bool BoneSpriteBase::playBecomeRedEffect()
{
...
float BoneSpriteBase::GetHideDistance()
{
	return 50;
}
