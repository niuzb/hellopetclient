...
function SkillSelectLayer:ShowTopSkillInfoItem()
	...
	if false == SkillDataCenter:IsUptoMaxLevel(skillId) and 0 ~= skillLevel then
		...
		if bTurnRed then
			...
			nextLevelHeroLevelLabel:setColor(ccc3(255,255,255))
		end
	else
...
function handleMsgAddUserSkillRsp(skill_id,skill_level)
	HideWaitingLayer()
	...
	--Note:升级成功
	SkillDataCenter:PushOneSkillItem(skill_id,skill_level)
