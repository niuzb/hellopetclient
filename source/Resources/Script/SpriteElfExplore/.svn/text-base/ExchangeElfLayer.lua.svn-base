...
function ExchangeElfLayer:ShowElfOnLayer(elfId)
	...
	if elfBgPic ~= nil then	
		...
		SpriteFactory:sharedFactory():ShowElfOnLayerAtPoint(container:getCurrentNode(),elfId,modePt)
	end
end
...
function ExchangeElfLayer:ShowExpLevelInfo(elfId)
	...
	local isExist = IsOneElfExistInElfDataTable(elfId)
...
function ExchangeElfLayer:FreshRightPanel()	
	...
	local isExist = IsOneElfExistInElfDataTable(id)
