	...
	if layout ~= nil then 
		...
		if attributeList ~= nil then
			...
			if (nameText ~= nil) and (desText ~= nil) and (number ~= nil)then
				local attrName,namePt = AspriteManager:getInstance():getOneFrameAndPosition("UI/ui2.bin","map_ui2_FRAME_ATTRIBUTE_TEXT_ATTRIBUTE");
				local attrNumber,numberPt = AspriteManager:getInstance():getOneFrameAndPosition("UI/ui2.bin","map_ui2_FRAME_ATTRIBUTE_NUMBER_ATTRIBUTE");
				local attrDes,desPt = AspriteManager:getInstance():getOneFrameAndPosition("UI/ui2.bin","map_ui2_FRAME_ATTRIBUTE_TEXT_ATTRIBUTE_DESCRIPTION");
				...
				if attributeControl[attri] == nil then
					...
					CCLuaLog(string.format("add attributeControl :%d",attri));
				else
					CCLuaLog(string.format("attributeControl :%d is exist",attri));
				end
			end
		end
	end
...
local function showUserAttributes()
...
local backPackLayout = TXGUI.UIManager:sharedManager():getUILayout("BackPack");
local attributeBtn = backPackLayout:FindChildObjectByName("playerAttributeBt").__UIButton__:getMenuItemSprite();
--local attributeCloseBtn = backPackLayout:FindChildObjectByName("attributeCloseBt").__UIButton__:getMenuItemSprite();
attributeBtn:registerScriptTapHandler(showUserAttributes);
--attributeCloseBtn:registerScriptTapHandler(hideUserAttributes);
...
local updateFunc = CCDirector:sharedDirector():getScheduler():scheduleScriptFunc(updateAttributes, 1.0, false);
local notificationFunc = NotificationCenter:defaultCenter():registerScriptObserver(updateNotification);

CCLuaLog(string.format("notificationFunc = %d",notificationFunc));
