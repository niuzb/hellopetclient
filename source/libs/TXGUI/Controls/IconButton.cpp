...
namespace TXGUI
{
		...
		cocos2d::CCObject * handler,SEL_MenuHandler pfnClicked)
	{
		...
		m_pagePosition = -1;
		m_priority = kCCScrollMenuPriority;
		if (m_backgroundTex)
		{
	...
	void IconButton::setItemLabel(const char * text,float fontSize,const char* font)
	{
		if(m_pButtonLabel == NULL)
		{
			...
			float x = getContentSize().width * 0.4;
			float y = getContentSize().height * 0.4;
			m_pButtonLabel->setPosition(ccp(x,-y));
			this->addChild(m_pButtonLabel, 2);
		}
	...
	void IconButton::setFrameSprite(CCSprite* sprite,CCPoint pt)
	{
		...
		{
			m_pFrameIcon = sprite;
			addChild(m_pFrameIcon, getZOrder()+1);
