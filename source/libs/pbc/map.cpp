...
_pbcM_si_new(struct map_kv * table, int size)
{
	...
	struct map_si * ret = (struct map_si *)_pbcM_malloc(sz);
	memset(ret,0,sz);
...
{
	struct map_ip * ret = (struct map_ip *)_pbcM_malloc(sizeof(struct map_ip));
	ret->array = NULL;
	ret->array_size = 0;
	ret->hash_size = (size_t)size;
	ret->slot = (struct _pbcM_ip_slot *)_pbcM_malloc(sizeof(struct _pbcM_ip_slot) * size);
...
_pbcM_ip_combine(struct map_ip *a, struct map_ip *b)
{
	...
	struct map_kv * table = (struct map_kv *)_pbcM_malloc(sz * sizeof(struct map_kv));
